    ...
    def _update_state_display(self):
        try:
            ...
            for session in sessions:
                ...
                html += (f"<tr><td>{session.get('name', 'N/A')}</td><td>{session.get('in_venue', 'N/A')}</td>"
                        f"<td>{session.get('hosted_by', 'N/A')}</td><td>{session.get('expected_attendees', 'N/A')}</td><td>{av}</td></tr>")
            if not sessions: 
