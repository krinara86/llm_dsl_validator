#
# Changes to: src/ui/interface.py
#
...
class ChatInterface:
    ...
    def _create_widgets(self):
        self.header = widgets.HTML("<h1>Event Management Assistant 🎯</h1><p>Manage venues and schedule sessions using natural language.</p>")
        available_models = self.system.get_available_models()
        self.model_selector = widgets.Dropdown(options=available_models, value=available_models[0] if available_models else None, description='Model:')
        self.role_selector = widgets.RadioButtons(options=['admin', 'scheduler'], value='admin', description='Role:')
        self.state_display = widgets.HTML()
        self.state_accordion = widgets.Accordion(children=[self.state_display], titles=('📊 Current State',))
        self.state_accordion.selected_index = None
        self.progress_label = widgets.HTML()
        self.chat_history = widgets.VBox([])
        self.user_input = widgets.Textarea(placeholder='Type your request or paste a document...', layout={'width': '95%', 'height': '80px'})
        self.send_button = widgets.Button(description='Send', button_style='success', icon='paper-plane')
        self.clear_button = widgets.Button(description='Clear', button_style='warning', icon='trash')
        self.confirm_button = widgets.Button(description='✅ Confirm', button_style='success', layout={'visibility': 'hidden'})
        self.cancel_button = widgets.Button(description='❌ Cancel', button_style='danger', layout={'visibility': 'hidden'})
    ...
    def display(self):
        ...
        layout = widgets.VBox([self.header, controls, widgets.HTML("<hr>"), self.state_accordion, self.progress_label, self.chat_history, input_area, confirm_area])
        display(layout)
        self._on_clear(None)
    ...
    def _on_send(self, b):
        ...
        if not query: return
        if len(query) > 300 and not self.conversation_state: self._process_document(query)
        else: self._process_query(query)
    ...
    def _on_confirm(self, b):
        ...
        self.chat_history.children = self.chat_history.children[:-1] # Remove "Executing..."
