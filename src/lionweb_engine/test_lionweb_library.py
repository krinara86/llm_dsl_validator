...
def test_lionweb_operations():
    ...
    # NEW: Ensure cycling_m2.json does not exist to test generation
    print("\n0. Preparing for test...")
    cycling_m2_path = project_root / "src" / "domains" / "cycling" / "cycling_m2.json"
    if cycling_m2_path.exists():
        cycling_m2_path.unlink()
        print(f"   - Deleted existing M2 language file to ensure it's regenerated.")
...
def display_language_info():
    ...
    print("\n" + "=" * 60)
    print("Language Information (Loaded from File)")
    print("=" * 60)
    
    from lionweb.language import Concept, Property
    
    # --- FIX ---
    # Explicitly define the project root so the loader finds the correct path
    project_root = Path(__file__).parent.parent.parent
    loader = LionWebConnectorLoader(project_root)
    # --- END FIX ---
...
if __name__ == "__main__":
    try:
        ...
        print("  ✅ Language M2 JSON created if not exists")
        print("  ✅ Language loaded from persistent M2 JSON file")
